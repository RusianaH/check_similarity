...
use Sastrawi\Stemmer\StemmerFactory;
    class Cosine {
        ...
        public function ambilJudul(){
            require 'config.php';
            $sql = mysqli_query($connection, "SELECT id,judul FROM files");
            $files = [];
            while ($result = mysqli_fetch_assoc($sql)){
        ...
        public function Dn(){
            ...
            $sql = mysqli_query($connection, "SELECT id,judul FROM files") or die(mysqli_error($connection));
            $files = [];
            while ($result = mysqli_fetch_assoc($sql)){
                array_push($files,$result);
            }
            $tbtoken = $this->ambilToken();
            foreach($files as $judul){
        ...
        public function Dfidf(){
            ...
            //mencari n
            $sql = mysqli_query($connection,"SELECT count(*) as total FROM files");
        ...
        public function train($sentence,$nama = null) {
                ...
                if($nama){
                    $sql = mysqli_query($connection, "INSERT into files (judul,nama) values('$sentence','$nama')");
                }
                // $sql = mysqli_query($connection, "INSERT into files (judul) values('$sentence')");
        ...
        private function tokenize($sentence) {
           $stopWords = array(
            ...
            'tentang', 'demi', 'dimana', 'kemana', 'pula', 'sambil', 'sebelum', 'sesudah', 'supaya',
             'kah', 'pun', 'sampai', 'sedang', 'selagi', 'sementara', 'tetapi', 'apakah',
            ...
            'sudah','untuk', 'dengan', 'kurang', 'jadi','maka','kapan','dimana','siapa','akan','dengan','supaya','jadinya','deh','yang','meng'
        );
        ...
        function cari($q){
            require './config.php';
            $sql = mysqli_query($connection, "SELECT nim,nama,judul,persen FROM files inner join tbcosine on files.id = tbcosine.idjudul
            WHERE judul = '$q'");
        }
